...
sub get_state {
    ...
    # need to look at html by prining $html, not by looking at source
    # or probably look at the html dump files that this program generates
    #Session time:</span><span class=value>12:03:38
    #<div id=flag class="scoring-header center flag-finish"><span class=label>&nbsp;</span><span class=value>Finish</span>
    #<title>Live Timing &amp; Scoring for 60th Annual Mobil 1 Twelve Hours of Sebring :: Race</title>

    ($session{time})      = $html =~ /Session time:.+?(\d\d\:\d\d\:\d\d)/;
    ($session{remaining}) = $html =~ /Remaining time:.+?(\d\d\:\d\d\:\d\d)/;
    ($session{flag})      = $html =~ /id\=flag.+?value>(\w+)<\/span/;
    ($session{event})     = $html =~ /<title>.+?\&amp\; (.+?)<\/title>/;
